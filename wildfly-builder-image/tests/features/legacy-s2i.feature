...
Feature: Wildfly Legacy s2i tests
  ...
  Scenario: Test provisioning.xml file
    Given s2i build https://github.com/jfdenise/wildfly-s2i from test/vanilla-wildfly/test-app-local-provisioning with env and True using ee-10-migration
      | variable                             | value         |
...
Scenario: Test external driver created during s2i.
  ...
  Scenario: Multiple deployments legacy
   Given s2i build http://github.com/jfdenise/wildfly-s2i from test/test-app-multi-deployments-legacy with env and True using ee-10-migration
   | variable                 | value           |
