...
Feature: Wildfly s2i tests
  ...
  Scenario: Test extension called at startup.
    Given s2i build https://github.com/jfdenise/wildfly-s2i from test/test-app-extension with env and true using wildfly-s2i-v2
    Then container log should contain WFLYSRV0025
  ...
  Scenario: Test execution of user CLI operations at S2I phase
    Given s2i build https://github.com/jfdenise/wildfly-s2i from test/test-app-s2i-cli-scripts with env and true using wildfly-s2i-v2
     | variable                               | value                                                 |
  ...
  Scenario: Test jaxrs-server +ejb-lite, -ejb-local-cache +ejb-dist-cache. Verify JGroups configuration added by ejb-dist-cache
    Given s2i build https://github.com/jfdenise/wildfly-s2i from test/test-app-ejb with env and True using wildfly-s2i-v2
      | variable                             | value                                                    |
    Then container log should contain WFLYSRV0025
    Then check that page is served
      | property              | value                                   |
